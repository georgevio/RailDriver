...
def load_raildriver_dll(dll_name=DLL_NAME):
    ...
    try:
        ...
        time.sleep(1)  # Delay maybe solves the raildriver loading error?
        return raildriver
    except OSError as e:
...
if __name__ == "__main__":
    ...
    if raildriver_lib:
        ...
        if loco_name:
            ...
            safe_loco_name = loco_name.replace(':', '_').replace('.', '_')
            ...
            with open(filename, "w") as outfile:
                ...
                # trying several times if errors...
                ...
                if controllers:
                    ...
                    log(1, f"Could not retrieve controller list. Is Train Simulator running a scenario?")
                    outfile.write("Could not retrieve controller list.\n")

            print(f"\nData written to: {filename}")

        else:
            print("Locomotive Name: Unknown")
            log(1, f"Could not retrieve locomotive name. Is Train Simulator running a scenarion?")
    else:
